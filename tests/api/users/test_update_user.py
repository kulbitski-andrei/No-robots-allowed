...
def test_update_user_profile_success(auth_token_and_user_data):
    ...
    assert response_data["firstName"] == update_data["firstName"], "First name did not update correctly"
    assert response_data["lastName"] == update_data["lastName"], "Last name did not update correctly"
    assert response_data["email"] == update_data["email"], "Email did not update correctly"
...
def test_update_user_profile_existing_email(auth_token_and_user_data):
    ...
    response = requests.post(f"{BASE_URL}/users", json=existing_user_data, headers=HEADERS)
    assert response.status_code == 201, f"Expected status 201, but got {response.status_code}"
    ...
    assert "name" in response_data and response_data["name"] == "MongoError", "Expected MongoError in the response data"
    assert "code" in response_data and response_data["code"] == 11000, "Expected error code 11000 for unique index violation"
    assert "keyValue" in response_data and response_data["keyValue"].get("email") == existing_email, "Expected existing email in the error details"
...
def test_update_user_profile_malformed_email(auth_token_and_user_data):
    ...
    assert response_data["_message"] == "User validation failed", "Unexpected '_message' value"
    ...
    assert response_data["errors"]["email"]["message"] == "Email is invalid", "Unexpected error message for 'email'"
    assert response_data["errors"]["email"]["value"] == "invalid-email-format", "Unexpected value for 'email' error"
...
def test_update_user_profile_weak_password(auth_token_and_user_data):
    ...
    assert "errors" in response_data, "Expected 'errors' in the response data"
    assert "password" in response_data["errors"], "Expected 'password' key in 'errors'"
    assert response_data["errors"]["password"]["message"] == "Path `password` (`short`) is shorter than the minimum allowed length (7).", "Unexpected error message for 'password'"
    assert response_data["errors"]["password"]["value"] == "short", "Unexpected value for 'password' error"
