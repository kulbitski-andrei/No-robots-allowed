...
def test_add_user_existing_email():
    ...
    response_data = response.json()
    assert "error" in response_data or "message" in response_data, "Expected an error message in the response"
...
def test_add_user_no_first_name():
    ...
    response_data = response.json()
    assert "error" in response_data or "message" in response_data, "Expected an error message in the response"
    assert "firstname" in response_data.get("message", "").lower(), "Expected error related to 'firstName'"
...
def test_add_user_invalid_email():
    ...
    response_data = response.json()
    assert "error" in response_data or "message" in response_data, "Expected an error message in the response"
    assert "email" in response_data.get("message", "").lower(), "Expected error related to 'email'"
...
def test_add_user_short_password():
    ...
    response_data = response.json()
    assert "error" in response_data or "message" in response_data, "Expected an error message in the response"
    assert "password" in response_data.get("message", "").lower(), "Expected error related to 'password'"
