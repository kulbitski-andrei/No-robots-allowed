...
def test_get_user_profile(auth_token_and_user_data):
    ...
    assert profile_data["email"] == user_data["email"], "The email in the profile does not match the registered email"
...
def test_get_user_profile_invalid_token():
    ...
    assert response_data["error"] == "Please authenticate.", "Expected 'Please authenticate.' message"
